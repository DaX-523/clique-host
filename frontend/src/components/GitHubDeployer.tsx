...
const baseUrl = "http://13.201.36.197:3001";
...
const GitHubDeployer: React.FC = () => {
  ...
  const handleDeploy = async () => {
    ...
    try {
      const response = await axios.post(baseUrl + "/api/deploy", { githubUrl });
      setDeploymentId(response.data.id);
      pollDeploymentStatus(response.data.id);
    } catch (err) {
      setError(
        "Failed to start deployment. Please check the GitHub URL and try again."
      );
      setIsLoading(false);
    }
  };
  ...
  const pollDeploymentStatus = async (id: string) => {
    try {
      const response = await axios.get(`${baseUrl}/api/status/?id=${id}`);
      ...
      if (response.data.status === "deployed") {
        ...
        setError(
          "Deployment is taking longer than expected. Please check the status later."
        );
        setIsLoading(false);
      }
    } catch (err) {
    ...
    <div className="bg-white rounded-lg shadow-xl p-8  w-full">
      ...
      <div className="mb-6">
        ...
        <p className="mt-2 text-sm text-gray-500">
          Make sure your repository contains a frontend project (e.g., React,
          Vue, Angular).
        </p>
      </div>
      <button
        ...
        className={`w-full py-3 px-4 rounded-md text-white font-medium transition duration-150 ease-in-out ${
          isValidGithubUrl(githubUrl) && !isLoading
            ? "bg-black hover:bg-black focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-black"
